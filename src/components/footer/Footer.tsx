...
const B = [
  ...
  const E = [
    { id: 1, title: 'Drama', path: '/#' },
    { id: 2, title: 'Horror', path: '/#' },
    { id: 3, title: 'Kids', path: '/#' },
    { id: 4, title: 'Romantic', path: '/#' },
    { id: 5, title: 'Comedy', path: '/#' },
  ];
  
  const I  = [
    { id: 1, title: '123 Nam Ki Khoi Nghia, Ngu Hanh Son, Da Nang, Viet Nam', path: '/#' },
  ...
  const A = ({ items }: { items: Array<{ id: number, title: string, path: string }> }) => {
    return (
      <ul className="text-gray-500 text-gray-400 font-medium text-left">
        {items.map(item => (
          <li key={item.id} className="mb-4">
            <Link to={item.path} className="hover:text-white">{item.title}</Link>
          </li>
        ))}
      </ul>
    );
  ...
  const renderCategory = (category: Category) => (
    <div key={category.title}>
      <h2 className="mb-12 text-sm font-semibold text-gray-900 uppercase text-white">
        {category.title}
      </h2>
  ...
  const renderSocialLink = (link: SocialLink) => (
    <a key={link.label} href={link.url} className="text-gray-500 hover:text-gray-900 dark:hover:text-white">
      {link.icon}
